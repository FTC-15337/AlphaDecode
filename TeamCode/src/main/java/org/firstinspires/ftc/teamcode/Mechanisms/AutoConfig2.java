...
public class AutoConfig2 {
    ...
    private double posX = 0;  // X position on the field (inches)
    private double posY = 0;  // Y position on the field (inches)
    private int lastDriveEncoder = 0;   // last reading of axial encoder
    private int lastStrafeEncoder = 0;  // last reading of lateral encoder
// "default" Maximum Yaw power limit during autonomous
    ...
    public PropControl driveController     = new PropControl(DRIVE_GAIN, DRIVE_ACCEL, DRIVE_MAX_AUTO, DRIVE_TOLERANCE, DRIVE_DEADBAND, false);
    public PropControl strafeController    = new PropControl(STRAFE_GAIN, STRAFE_ACCEL, STRAFE_MAX_AUTO, STRAFE_TOLERANCE, STRAFE_DEADBAND, false);
    public PropControl yawController       = new PropControl(YAW_GAIN, YAW_ACCEL, YAW_MAX_AUTO, YAW_TOLERANCE,YAW_DEADBAND, true);
    ...
    // Robot Constructor
    public AutoConfig2(LinearOpMode opmode) {
        myOpMode = opmode;
    }
    ...
    public boolean readSensors() {
        ...
        int currentDrive = driveEncoder.getCurrentPosition() * (INVERT_DRIVE_ODOMETRY ? -1 : 1);
        int currentStrafe = strafeEncoder.getCurrentPosition() * (INVERT_STRAFE_ODOMETRY ? -1 : 1);

        // Calculate change since last loop
        int deltaDrive = currentDrive - lastDriveEncoder;
        int deltaStrafe = currentStrafe - lastStrafeEncoder;

        lastDriveEncoder = currentDrive;
        lastStrafeEncoder = currentStrafe;

        // Convert to inches
        double deltaDriveInches = deltaDrive * ODOM_INCHES_PER_COUNT;
        double deltaStrafeInches = deltaStrafe * ODOM_INCHES_PER_COUNT;
        ...
        // Rotate incremental movement into field coordinates
        double headingRad = Math.toRadians(heading);
        posX += deltaDriveInches * Math.cos(headingRad) - deltaStrafeInches * Math.sin(headingRad);
        posY += deltaDriveInches * Math.sin(headingRad) + deltaStrafeInches * Math.cos(headingRad);

        // Update distances for your controllers
        driveDistance  = posX;  // distance from start along X
        strafeDistance = posY;  // distance from start along Y
    ...
    public void resetOdometry() {
        // Store current encoder counts as “zero”
        lastDriveEncoder = driveEncoder.getCurrentPosition();
        ...
        // Reset heading
        readSensors();  // updates rawHeading
...
class PropControl {
    ...
    public PropControl(double gain, double accelLimit, double outputLimit, double tolerance, double deadband, boolean circular) {
        this.gain = gain;
